...
| ID     | Description                                                                                                                                                                                              |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SRS-01 | The STM32 shall read the IMU accelerometer data over SPI at a minimum rate of 50 Hz to enable accurate step detection.                                                                                  |
...
| SRS-05 | The display shall cycle through atleast three different modes, such as: time view, step count view, and compass view.                                                                                    |
...
| ID     | Description                                                                                                                                                                                                                   |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HRS-01 | The system shall use an IMU with atleast 3-axis accelerometer, 3-axis gyroscope, and 3-axis magnetometer capabilities, communicating over SPI, to provide motion data for step-count and compass data for direction.         |
...
Hardware & Firmware System: The hardware in the final product includes an STM32F411CEU6 (STM32 Black Pill) for its small form factor. One of the five SPI peripherals, specifically SPI4, is used to communicate and display information on the DFR0934 OLED, which served as our main output device. The glasses had two main input devices. The first, using the SPI2 peripheral, is the LSM6DS0 IMU that records the step count. The second, using an ADC, is a photoresistor divider that captures the ambient light intensity, which allows the system to automatically adjust OLED brightness. Using the step count, the speed is also approximated (in MPH) and displayed onto the OLED screen. To interface with the smart glasses, four buttons are used to trigger external interrupts via GPIO pins. Finally, to power the glasses, a 1s Lipo battery was stepped down to 3.3V using the MCP1700 LDO, which had a typical dropout of 178mV.

To keep the firmware organized, the drivers and code for the SPI peripherals, OLED, ADC, etc. were managed in different files. The main.c file then connects these different files to manage the main GUI. The GUI consisted of 5 distinct screens that the user could cycle through: the welcome screen, time screen, step-count screen, countdown timer screen, and brightness control screen. The welcome screen displays a welcome message. The time screen displays the time, which is calibrated by an external computer upon flashing the code. The step-count screen displays both the speed (in MPH) and step count. The timer screen has a timer which can be adjusted with minute-level precision. Finally, the brightness control screen has 4 brightnesses: low, medium, high, and automatic. Automatic brightness control utilizes the photoresistor and ADC to adjust the brightness.

#### 3.1 Software Requirements Specification (SRS) Results

| ID     | Description                                                                                                                                                                               | Validation Outcome                                                                                                                                                                                                                                                                                 |
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SRS-01 | The STM32 shall read the IMU accelerometer data over I2C or SPI at a minimum rate of 50 Hz to enable accurate step detection.                                                             | Partially Achieved. The LSM6DS0 6-axis IMU's step data was accurately measured over SPI. However, while desoldering/re-soldering the STM32 to fit the tight space of the glasses enclosure, the SPI peripheral for the IMU was broken for the final product.                                      |
...
| ID     | Description                                                                                                                                                                      | Validation Outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HRS-01 | The system shall use an IMU with at least 3-axis accelerometer, 3-axis gyroscope, and 3-axis magnetometer capabilities, communicating over I2C, for step-count and compass data. | Partially Achieved. Instead of a 9-axis IMU, a 6-axis IMU successfully worked using SPI.                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| HRS-02 | The display shall be small enough to mount on glasses frames and support at least two lines of text output.                                                                      | Achieved. More than two lines of text output was supported and the display was small enough to mount on glasses frames as **validated** in the demo video.                                                                                                                                                                                                                                                                                                                                                                    |
| HRS-03 | Several push buttons shall be mounted on the glasses frame and register a press within 100ms of actuation to enable mode toggling, parameter changes, and power on/off.          | Partially achieved. The buttons are responsive as per the demo video. However, there's a 300ms hardcoded delay to prevent debouncing.                                                                                                                                                                                                                                                                                                                                                                                                 |
| HRS-04 | Two ESP32 modules shall maintain wireless connection with a round-trip latency of less than 500ms within 5m of distance.                                                         | Not relevant. ESP32 not used.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| HRS-05 | The STM32 shall communicate with the base ESP32 over UART at a baud rate of at least 9600 bps.                                                                                   | Not relevant. ESP32 not used.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| HRS-06 | The glasses shall be powered by a battery capable of powering the glasses for at least 15–30 minutes continuously.                                                              | Achieved.**Validated** by the specs of the system. STM32 draws ~45mA, OLED ~15mA, IMU ~5mA, Photoresistor ~3mA. <br />These are the upper current-draw values from the datasheets. Additionally, the photoresistor is in series with a 1k ohm resistor, so it has an absolute max draw of 3.3V/1000 = 3.3mA.<br />The LP552530 1s Lipo Battery has 350mAh capacity. <br />With a max total current draw of ~70mA, **350mAh/70mA = 5 hours. This is signficantly above the HRS of 30 minutes even assuming worst-case.** |
